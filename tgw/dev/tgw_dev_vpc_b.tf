...
resource "aws_ec2_transit_gateway_vpc_attachment" "tgw_attach_vpc_b" {
  subnet_ids         = data.terraform_remote_state.dev_vpc_all.outputs.public_subnet_ids_b #module.vpc_b.public_subnets
  transit_gateway_id = aws_ec2_transit_gateway.tgw.id
  vpc_id             = data.terraform_remote_state.dev_vpc_all.outputs.vpc_id_b #module.vpc_b.vpc_id
  tags = {
    Name = "Transit Gateway VPC B Attachment - Dev"
  }
...
resource "aws_route" "private_b_to_a" {
  for_each               = toset(data.terraform_remote_state.dev_vpc_all.outputs.private_route_table_ids_b) #toset(module.vpc_b.private_route_table_ids)
  route_table_id         = each.value
  destination_cidr_block = data.terraform_remote_state.dev_vpc_all.outputs.cidr_block_a #module.vpc_a.vpc_cidr_block
  transit_gateway_id     = aws_ec2_transit_gateway.tgw.id
...
resource "aws_route" "public_b_to_a" {
  for_each               = toset(data.terraform_remote_state.dev_vpc_all.outputs.public_route_table_ids_b) #toset(module.vpc_b.public_route_table_ids)
  route_table_id         = each.value
  destination_cidr_block = data.terraform_remote_state.dev_vpc_all.outputs.cidr_block_a #module.vpc_a.vpc_cidr_block
  transit_gateway_id     = aws_ec2_transit_gateway.tgw.id
