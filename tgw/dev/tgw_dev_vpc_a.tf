...
resource "aws_ec2_transit_gateway_vpc_attachment" "tgw_attach_vpc_a" {
  subnet_ids         = data.terraform_remote_state.dev_vpc_all.outputs.public_subnet_ids_a #module.vpc_a.public_subnets
  transit_gateway_id = aws_ec2_transit_gateway.tgw.id
  vpc_id             = data.terraform_remote_state.dev_vpc_all.outputs.vpc_id_a #module.vpc_a.vpc_id
  tags = {
    Name = "Transit Gateway VPC A Attachment - Dev"
  }
...
resource "aws_route" "private_a_to_b" {
  for_each               = toset(data.terraform_remote_state.dev_vpc_all.outputs.private_route_table_ids_a) #toset(module.vpc_a.private_route_table_ids)
  route_table_id         = each.value
  destination_cidr_block = data.terraform_remote_state.dev_vpc_all.outputs.cidr_block_b #module.vpc_b.vpc_cidr_block
  transit_gateway_id     = aws_ec2_transit_gateway.tgw.id
...
resource "aws_route" "public_a_to_b" {
  for_each               = toset(data.terraform_remote_state.dev_vpc_all.outputs.public_route_table_ids_a) #toset(module.vpc_a.public_route_table_ids)
  route_table_id         = each.value
  destination_cidr_block = data.terraform_remote_state.dev_vpc_all.outputs.cidr_block_b #module.vpc_b.vpc_cidr_block
  transit_gateway_id     = aws_ec2_transit_gateway.tgw.id
